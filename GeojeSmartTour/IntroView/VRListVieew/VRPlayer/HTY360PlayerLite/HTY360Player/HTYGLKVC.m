...
- (void)update
{
    ...
    GLKMatrix4 projectionMatrix = GLKMatrix4MakePerspective(GLKMathDegreesToRadians(_overture), aspect, 0.1f, 400.f);
    projectionMatrix = GLKMatrix4Rotate(projectionMatrix, ES_PI, 1.f, 0.f, 0.f);
    
    GLKMatrix4 modelViewMatrix = GLKMatrix4Identity;
    modelViewMatrix = GLKMatrix4Scale(modelViewMatrix, 300.f, 300.f, 300.f);
    if(_isUsingMotion) {
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    for (UITouch *touch in touches)
    {
        ...
        //        NSLog(@"touchPos.x : %f",touchPos.x);
        //        NSLog(@"touchPos.y : %f",touchPos.y);
    }
}
...
{
    [_videoPlayerController playOrResume];
}
